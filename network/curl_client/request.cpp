...
namespace curl_client {
  ...
  Request* Request::pathSetNew(std::string&& p) {
	  auto new_path = std::make_unique<PathData>(std::move(p));
  ...
  Request* Request::pathPushBackElement(std::string &&elem) {
	  auto new_elem = utils::splitIntoWords(elem, path->delim);
  ...
  void Request::returnText() {
	  response->body = curl->getResponseBody();
  }
  void Request::returnError() {
	  response->body = curl->getError();
  }
  void Request::returnJson() {
	  ...
	  response->body = Json::Load(json_to_str);
  }
...
//		  return;
	  ...
	  std::visit(utils::overloaded {
			  [&os]([[maybe_unused]] std::monostate arg) { os << "monostate\n"; },
