...
#define DEBUG_OUPUT
  ...
  void CurlHandler::setHeaders(const HttpHeaders &hdrs){
	  if (curl_handle) {

		  if (not hdrs.empty()) {
			  for (const auto &[_, header] : hdrs) {
...
/* A less good option would be to enforce HTTP 1.0, but that might also have other implications. */
{
  ...
  void CurlHandler::setMethod (Method new_method) {
	  if (not isSetMethod(new_method)) updateMethod(new_method);
  }
  ...
  size_t CurlHandler::read_callback(char *buffer, size_t size, size_t nmemb, void *userdata){
	  ...
	  auto *q_data = (struct QueryData *)userdata;
  ...
  */
  ...
  void CurlHandler::updateMethod(const Method new_method){
	  (this->*set_method.at(new_method))();
